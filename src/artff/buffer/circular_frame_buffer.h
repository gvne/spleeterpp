...
public:
  ...
  std::complex<float>* Push(const std::complex<float> *data);
  void Pop(std::complex<float>* data);
  void Reset();
  void Resize(uint32_t fft_size, uint32_t frame_count);

private:
  // Each frame_count * fft_size * channel_count
  std::vector<std::vector<std::complex<float>>> m_storage;
