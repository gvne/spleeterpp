#ifndef SPLEETER_FILTER_TENSOR_HELPER_H_
#define SPLEETER_FILTER_TENSOR_HELPER_H_
...
void SetFrame(TFHandlePtr<TF_Tensor> tensor, std::vector<int64_t> shapes,
              uint32_t frame_index, std::vector<const T *> data) {
...
/// \param shapes the tensor shapes
template <typename T>
void GetFrame(const std::vector<T *>& data, uint32_t frame_index,
...
// --- Helper to get the TF_DataType from a template type
template <typename T>
struct TensorType {};

template <>
struct TensorType<float> {
  static TF_DataType data_type() { return TF_DataType::TF_FLOAT;}
};

template <>
struct TensorType<std::complex<float>> {
  static TF_DataType data_type() { return TF_DataType::TF_COMPLEX;}
};

} // namespace spleeter

#endif // SPLEETER_FILTER_TENSOR_HELPER_H_
